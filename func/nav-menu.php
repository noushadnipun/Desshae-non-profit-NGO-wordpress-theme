...
function register_my_menu() {
  register_nav_menu( 'header-main-menu', __( 'Header Main Menu', 'theme-slug' ) );
}
add_action( 'after_setup_theme', 'register_my_menu' );

function add_classes_on_li($classes, $item, $args) {
  $classes[] = 'nav-item hs-has-sub-menu u-header__nav-item';
  return $classes;
}
...
if ( !class_exists( 'WP_Bootstrap_Navwalker' ) ) {
  ...
  class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
    ...
    public function start_lvl( &$output, $depth = 0, $args = array() ) {
      ...
      $this->dropdown = true;
      $output         .= $n . str_repeat( $t, $depth ) . ' <ul id="homeSubMenu" class="list-inline hs-sub-menu u-header__sub-menu mb-0 animated fadeInUp style="min-width: 220px;" aria-labelledby="homeMegaMenu"><li class="dropdown-item hs-has-sub-menu">' . $n;
    }
    ...
    public function end_lvl( &$output, $depth = 0, $args = array() ) {
      ...
      $this->dropdown = false;	  
      $output         .= $n . str_repeat( $t, $depth ) . '</li></ul>' . $n;
    }
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
      ...
      $item_classes = array( 'nav-link u-header__nav-link' );

      if ( $args->walker->has_children ) { 	     
        $item_classes = array( 'nav-link' );
		$item_classes[] = 'u-header__nav-link';		
      }	  
      ...
      if ( 0 < $depth ) {
		...
		$item_classes = array( 'nav-link' );
        $item_classes[] = 'u-header__sub-menu-nav-link';
      }
     
	  $item_output = $args->before;
      $item_output .= '<a class="' . implode( ' ', $item_classes ) . '" ' . $attributes . '>';
      $item_output .= $args->link_before . $title . $args->link_after;
	  $item_output .= ($depth == 0 && $args->walker->has_children) ? '<span class="fa fa-angle-down u-header__nav-link-icon"></span></a>' : '</a>';
      //$item_output .= '</a>';
